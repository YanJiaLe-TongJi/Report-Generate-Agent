...
def export(database, uploads, output):
    ...
    with sqlite3.connect(Path(database).resolve().as_uri()+'?mode=ro', uri=True) as conn:
        ...
        rows = conn.execute('SELECT experiment_name, material_paths, example_path FROM system_materials WHERE is_active=1').fetchall()
    for row in rows:
        ...
        for raw in paths:
            ...
            records.append((source, row['experiment_name']))
    if not records: raise ValueError('没有可导出的有效资料')
    with zipfile.ZipFile(output, 'w', zipfile.ZIP_DEFLATED) as archive:
        manifest = []
        for index, (source, experiment) in enumerate(records):
            name = f'files/{index:04d}/{source.name}'
            archive.write(source, name)
            manifest.append({'path': name, 'experiment': experiment})
